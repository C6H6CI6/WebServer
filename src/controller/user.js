...
async function getUserDepositTokens(address) {
  var request = {
    caller: address,
    method: 'get_deposit',
    payload: '',
  ...
  try {
    ...
    for (var i = 0; i < items.length; i++) {
      if (items[i].name == name) {
        const user = JSON.parse(JSON.stringify(items[i].toJSON()));
        user.error = 0;
        var tokens = await getUserTokens(user.muta_address);
        user.n_tokens = tokens;
        user.d_tokens = await getUserDepositTokens(user.muta_address);
    ...
    User.find({ name: req.session.logged_in.name }).then(async function (items) {
      ...
      for (var i = 0; i < items.length; i++) {
        if (items[i].name == req.session.logged_in.name) {
          ...
          items[i].d_tokens = await getUserDepositTokens(items[i].muta_address);
          res.json({
            error: 0,
            data: items[i]
          });
